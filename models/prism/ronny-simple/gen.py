...
def generateModel(threadCount, workTicks, readTicks, writeTicks, debug) :
	...
	for i in range(0, threadCount) :
		s_, t_ = shared_memory.generateVariable("bar_%d" % i, "[0..1]",  "0", ["0", "1"], threadCount, debug, False)
		s += s_
		t += t_
...
def generateThread(p, threadCount) :
	...
	s += "    [bar_#_set_to_1_#]            l_#=l_write                       ->        (l_#'=l_wait);\n"

	s += "\n"
	s += "    [does_i_equal_thread_count_#] l_#=l_wait  & i_# = thread_count  -> tick : (l_#'=l_done);\n"
	for i in range(0, threadCount) :
		s += "    [bar_%d_read_#]                l_#=l_wait  & i_# = %d & bar_%d = 1 ->        (i_#' = min(i_# + 1, thread_count));\n" % (i, i, i)
		s += "    [bar_%d_read_#]                l_#=l_wait  & i_# = %d & bar_%d = 0 ->        (i_#' = 0);\n" % (i, i, i)
	s += "\n"
	s += "    [done_#]                      l_#=l_done                        -> rare : true;\n"

	s += "\n"
	s += "    // do never trigger the following sync transitions\n"
	for i in range(0, threadCount) :
		s += "    [bar_%d_atomic_begin_#] false -> true;\n" % i
		s += "    [bar_%d_atomic_end_#]   false -> true;\n" % i
		s += "    [bar_%d_set_to_0_#]     false -> true;\n" % i
		if i != p :
			s += "    [bar_%d_set_to_1_#]     false -> true;\n" % i
...
def operationRewards(guard, threadCount) :
	...
	s += "rewards \"%s\"\n" % sharedRewardName
	for p in range(0, threadCount) :
		s += "    [bar_%d_set_to_1_%d]            %s & bar_%d_shared_write_%d : %d;\n" % (p, p, guardName, p, p, 1)
		for q in range(0, threadCount) :
			s += "    [bar_%d_read_%d]                %s & bar_%d_shared_read_%d  : %d;\n" % (q, p, guardName, q, p, 1)
	s += "endrewards\n"

	# module thread_#\n"
	#     [work_#]                      l_#=l_work                        -> work : (l_#'=l_write)    # (local ops: 0, shared ops: 0)
	#
...
def generateLabels(threadCount) :
	...
	s += "formula exactly_one_is_done_working  = " +  " | ".join([ "(l_%d >= l_write & " % i + " & ".join(["l_%d = l_work" % j for j in range(0, threadCount) if j!=i]) + ")" for i in range(0, threadCount)]) + ";\n"
	...
	s += "formula one_is_writing               = !all_are_working & (" + " | ".join([ "l_%d <= l_write" % i for i in range(0, threadCount)]) + ");\n"
	s += "formula one_is_writing_inc           = " + " | ".join([ "l_%d <= l_write" % i for i in range(0, threadCount)]) + ";\n"
	#s += "formula all_are_writing              = " + " & ".join([ "l_%d = l_write" % i for i in range(0, threadCount)]) + ";\n"
