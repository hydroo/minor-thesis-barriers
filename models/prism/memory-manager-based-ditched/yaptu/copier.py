...
class Copier:
    ...
    def copyblock(self, i=0, last=None):
        ...
        def repl(match, self=self):
            """1. preprocess match 2. eval the result"""
        ...
        while i<last:
            ...
            if match:   # a statement starts "here" (at line block[i])
                ...
                stmt = match.string[match.end(0):].strip()
                ...
                while j<last:
                    ...
                    if self.restend.match(line):    # found a statement-end
                        ...
                        if match:                   # found a contin.-statement
                            nestmt = match.string[match.end(0):].strip()
                            stmt = '%s _cb(%s,%s)\n%s' % (stmt,i+1,j,nestmt)
                            i=j     # again, i is the last line to _not_ process
                    j=j+1
                stmt = self.preproc(stmt, 'exec')
                # this construct needs : or # as stmt end
                stmt = '%s _cb(%s,%s)' % (stmt,i+1,j)
                # for debugging, uncomment...: print "-> Executing: {"+stmt+"}"
...
#
#    >> for i in range(5):
#    some text $i$ with newline
#    <<
#    
#    >> for i in range(5): "some text"+str(i)<<
#
#    >> if i<5:
#    some text
#    > else:
...
#    [[if i<5:]
#    some text
#    [[while false:]nothing[else:] something]]
#    [else:]
#    another text]]
#
#    block := [[ stmt sep stuff (stmt sep stuff)* ]]
#    sep := ]\n?
#    stuff := block | text