...
\begin{itemize}
	...
	\item bit shuffling easier to implement in Spin. C-esque style
	\item resetting and repetition only needed for functional properties
	\item used Spin for functional properties
	\item PRISM model more minimized and specialized for model checking our wanted quantitative properties
	\item CTMCs will be composed of multiple modules.
	...
	\item modules are executed interleaved except if transitions require synchronizing actions. A synchronizing action is an action that can be issued by at least two modules. It can only fire if all modules, that are able to fire this action eventually, are presently in a state where they can fire it. The rate is determined by any one module. For each synchronization action, in our case, only one module will specify a rate for this action.
...
\begin{itemize}
	...
	\item Control-flow diagram augmented with rates: Figure~\ref{fig:dissemination-control-flow}
		...
		\end{figure}
	\item As described in Section~\ref{sssec:analysis-modelchecking-modelling-central-counter} $\lambda_{work}$ is the work rate.
	\item $\lambda_t$ is the rate for one CPU cycle
	\item $\lambda_r$ is the rate for a local read
	\item $\lambda_p$ is the rate for a remote write, usually called \textit{put}
	\item usually we want $\lambda_t = \lambda_r$ and $\lambda_p \gg \lambda_r$
	...
	\item \todo talk about synchronization between processes in the CTMC
	\item the functional model implements resetting and repetition using a counter instead of booleans. The general approach is described in Section~\ref{ssec:building-blocks} (if you did not describe it there, you have to do it here)
\end{itemize}
