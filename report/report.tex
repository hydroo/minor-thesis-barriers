...
In our analysis we will only scratch the surface of what the interface has to offer, since diving deep involves lots of details, complicated semantics and many of the details don't matter for our purpose.
...
Furthermore MPI facilitates asynchronous message passing. One can send receive messages without blocking and one can test if a message has been successfully transmitted. Synchronous and asynchronous message passing may be mixed. That means one can, for example, issue a non-blocking send that is matched by a blocking receive operation.
...
The definition of RMA according to the standard is pretty involvement, and we have to go into a bit more detail now to highlight important points. (mittelguter satz)

To initiate sharing remote memory each participating process issues a collective call to create a \emph{window} of memory to be shared. A window is associated with the group of processes that created it, and through it each participating process exposes a window of local memory to the other peers that can be used for remote memory operations.
...
MPI differentiates between \emph{active} and \emph{passive} target communication. Active target communication involves both peers being actively involved in windows synchronization, whereas in passive only one side's activity is needed.

One way to synchronize a window is via a \emph{fence} operation. A fence is a collective operation among all window group members that makes sure that all outstanding RMA requests on this window will be finished before the operation returns. It implies a full barrier synchronization.

A more fine-grained instance of active target communication is via the operations \emph{start}, \emph{complete}, \emph{post} and \emph{wait}. The remote memory access calls issuing process calls start to begin his transmission and complete to end it, whereas post and wait are used by the remote command receiving process. Additionally one can specify the processes involved in this synchronization, which do not need to be the whole window group like for fence operations.
