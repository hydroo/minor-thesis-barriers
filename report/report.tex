...
\begin{itemize}
	...
	\item The reachability reward is similar to the accumulated reward. It is defined as
		\begin{center}
			$\mathrm{ReaRew}(\pi, \Phi) = \mathlarger{\sum}\limits_{i=0}^{\mathit{min}\{j | s_j \models \Phi~\wedge~0 \le j < n\}-1}} (t_{i+1} - t_i) \cdot \mathit{rew}(s_i)$
		\end{center}
	...
	\item in the analysis of the protocols we will for example deal with the reward function that assigns value base\_rate (or 1) to each state. In this case $\mathrm{ReaRew}(T)$ can be interpreted as the average cycle count to reach $T$.
\end{itemize}
...
\begin{itemize}
	\item distinct models for both analysis of functional and quantitative properties
	...
	\item ordinary (non-stochastic) transition systems for function models and CTMCs for quantitative models
	...
	\item synchronization between modules can either be done indirectly through modifying variables in the program graph that another modules uses as well, or using synchronizing actions.
	...
	\item If no other modules uses $\alpha$ \\
		\begin{center}
...
\begin{itemize}
	...
	\item Control flow diagram Figure~\ref{fig:central-counter-control-flow}
		\begin{figure}[htbp]
			\centering
			\input{tikz/central-counter-no-reset}
...
\begin{itemize}
	...
	\item Control flow diagram Figure~\ref{fig:ronny-simple-control-flow}
		\begin{figure}[htbp]
			\centering
			\input{tikz/ronny-simple-no-reset}
...
\begin{itemize}
	...
	\item Control flow diagram augmented with rates: Figure~\ref{fig:dissemination-control-flow}
		\begin{figure}[htbp]
			\centering
			\input{tikz/dissemination-no-reset}
	...
	\item we assume all local operations to be much quicker, ten to a thousand times, than remote operations. Therefore we assign the rate $\lambda_t$ (one CPU cycle) to local operations. $\lambda_p$, which is assigned to remote write (put) operations, gets a lower rate.
...
\begin{itemize}
	...
	\item Control flow diagram augmented with rates: Figure~\ref{fig:ronny-fancy-control-flow}
		\begin{figure}[htbp]
			\centering
			\input{tikz/ronny-fancy-no-reset}
			\caption{Augmented Control flow diagram of ronny's fancy barrier}
			\label{fig:ronny-fancy-control-flow}
		\end{figure}
	\item Rates are like in previous section, except we now use remote read (get) with a rate of $\lambda_g$.
...
\begin{itemize}
	...
	\item $done_t$ is a predicate describing states where thread $t$ has passed the barrier
	\item $work\_done_t$ is a predicate describing states where thread $t$ has passed the initial work period
	\item \textbf{Q1:} ''\emph{a thread may only exit the barrier if all threads have entered it}''
		\begin{center}
			$\square \big( ~ ( \exists t \in T . \mathit{done_t} ) \implies \forall s \in T. \mathit{work\_done_s} ~ \big)$
		\end{center}
	\item \textbf{Q2:} ''\emph{if all threads entered the barrier, they will all leave it in a finite amount of time}''
		\begin{center}
			$\square \big( ~(\forall t \in T . \mathit{work\_done_t} ) \implies \forall s \in T. \lozenge \mathit{done_s} ~ \big)$
		\end{center}
	\item \textbf{Q1} and \textbf{Q2} are the only two properties a barrier protocol must satisfy
	\item \textbf{Q1} could be seen as safety and \textbf{Q2} as liveness of the protocol
	\item yes it works.
\end{itemize}
