...
% \begin{itemize}
% 	\item maybe use use nicer colors for diagrams
...
At a recent workshop, Nicholas Mc~Guire proposed a new low-level synchronisation scheme, called probabilistic Write/Copy-Select (short pW/CS)~\cite{pwcs} lock. It aims to improve the performance of exclusive access to shared memory. The core ideas, though, apply to synchronisation in general:
\begin{itemize}
...
Two more drawbacks of measurement is that the granularity of the measurement scale is limited, and that measuring itself can influence the outcome. This makes analysing low-level synchronisation primitives, and small program pieces in general, hard and sometimes impossible.
...
Model checking, therefore, complements measurement-based analysis.
...
The final section summarises the presented work and gives an outlook of possible directions for future research.
...
We then present three new protocols, one of which shows promise. We therefore conclude that barrier protocols can be improved. To achieve the displayed performance, the B2 Barrier does not use a fixed communication pattern but relies on randomness. Weakening determinism and using randomness are core principles of pW/CS locks. The B2 Barrier can, therefore, be considered an example for the aptitude of the ideas behind pW/CS locks to improve synchronisation performance.
Using model checking enabled a fine-grained analysis beyond what measurement-based methods can achieve.
...
One should repeat the analysis and comparison done in Section~\ref{sssec:analysis-modelchecking-functional-properties} using measurement instead of model checking. Once this is done, the models should also be refined.

Second, new barriers and variations of old ones should be evaluated.
For example one can try different increment strategies for the iteration variable of the B2 Barrier.
The B1 and B2 Barrier both exclusively read remotely and write locally. In particular, both busily wait on remote memory. Some hardware-platforms, like the Epiphany~\cite{epiphany} multi-core processor, also offer better performance for remote write operations than for remote read operations.
By reversing the situation so that remote writing in conjunction with local busy waiting is used, one might be able to get rid of busy waiting on remote memory and leverage the potentially better performance of remote write operations on some architectures.
...
Another interesting pursuit is to model check in greater detail. Today's processors, for example, have a multi-layered cache. In order to get preciser performance predictions one can incorporate these intricacies into the model.
