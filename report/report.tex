...
\begin{itemize}
	...
	\item time
		\begin{itemize}
			...
			\item work is equal for everyone, because the distribution is the same for all.
			\item first thread leaves a few cycles after writing is done. Because the last one to write its value will likely immediately read the completed barrier and leave.
...
\begin{itemize}
	...
	\item dissemination: distribution of total time spent: Figure~\ref{fig:d-time-work-100-partition}
		...
		\end{itemize}
	\item progress problem of the dissemination barrier: Figure~\ref{fig:df-time-work-1000-partition}, Figure~\ref{fig:d-time-work-1000-partition}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/df-time-work-1000-partition}
			\caption{Distribution of total time spent}
			\label{fig:df-time-work-1000-partition}
		\end{figure}
		\begin{figure}[htbp]
			\centering
		...
		\end{figure}
		\begin{itemize}
			\item emulated by work rate $1/1000$
			\item dissemination has to wait for each round to finish before making more progress
			\item ronny's fancy is considerably faster for process counts above 2
			\item ronny's fancy cannot be modelled for process counts that are needed to quantify this properly. Model checking restrictions.
			\item a third round adds about 200 more cycles to the runtime
		\end{itemize}
