...
\begin{itemize}
	...
	\item The reachability reward is similar to the accumulated reward. It is defined as
		\begin{center}
			$\mathrm{ReaRew}(\pi, \Phi) = \mathlarger{\sum}\limits_{i=0}^{\mathit{min}\{j | s_j \models \Phi~\wedge~0 \le j \le n\}-1}} (t_{i+1} - t_i) \cdot \mathit{rew}(s_i)$
		\end{center}
...
\begin{itemize}
	...
	\item \textbf{Q2:} ''\emph{if all threads entered the barrier, they will all leave it in a finite amount of time}''
		...
		\end{center}
	\item Q1 and Q2 are the only two properties a barrier protocol must satisfy
	\item Q1 could be seen as safety and Q2 as liveness of the protocol
	\item yes it works.
\end{itemize}
	...
	\item which parameters to variate
		\begin{itemize}
			...
			\item work rate
				...
				\end{itemize}
		\end{itemize}
	\item properties for all protocols
		\begin{itemize}
			\item interesting points in time
				\begin{itemize}
					\item (A) first entered the barrier
					\item (B) last entered the barrier
					\item (C) first left the barrier
					\item (D) last left the barrier
				\end{itemize}
		\end{itemize}
	\item shared memory only
		\begin{itemize}
			\item time spent writing vs reading
		\end{itemize}
	\item distributed memory only
		\begin{itemize}
			\item progress
			\item number of remote transfers as another performance measure
		\end{itemize}
\end{itemize}
...
\paragraph{Results}
	\begin{itemize}
		\item central counter vs ronny's simple
			\begin{itemize}
				\item expected time (last in - last out) (x: threads, y: expected time)
				\item expected time (first out - last out) (x: threads, y: expected time)
			\end{itemize}
