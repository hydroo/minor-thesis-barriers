...
For convenience we use combinations of action labels like $\alpha \lor \beta$ meaning that there are two transitions, one with action $\alpha$ and one with $\beta$ between two states. Either action can trigger the transition.
...
The execution times of the cache line copy state transitions are as follows.
...
If a thread reads a variable whose copy is invalid, it has to advertise its intent to all other copies. A copy that is modified then needs to switch to the shared state. Meanwhile the thread fetches an up-to-date copy from main memory, and sets its local copy state to shared. After all of this is done, it continues program execution. This usually takes around 50 processor clock cycles.
...
The Dissemination Barrier (Figure~\ref{fig:pseudocode-dissemination}, Section~\ref{sssec:background-currently-used-distributed}), as a representative for distributed barrier algorithms, does not have shared memory. Therefore the CTMC model consists of one process module per process and nothing else.
...
One could model the algorithm more accurate by splitting some of the transitions into multiple transitions. We refrain from this to keep the model size at a manageable level.
...
The functional model includes repetition and resetting with a counter instead of boolean variables as described in Section~\ref{ssec:background-building-blocks}.
