...
\begin{itemize}
	\item review all captions again
...
It has to properly synchronise threads, i.e. no thread leaves before all threads arrived at the barrier, and may not deadlock.
...
First we assume the initial work period to take 100 cycles on average, which is extremely low considering that barrier synchronisation takes more time than the actual computation task. Later we switch to larger work periods.

Figure~\ref{fig:c1-time-work-100-B-C-D} presents the basic timing of our two shared memory barriers for different thread counts.
The time from the last thread to enter the barrier to the last thread to leave the barrier (dark red) is similar for both the Central Counter Barrier and the B1 Barrier.
...
During B there is at least one thread that has not entered the barrier. When the last thread enters, we reach the writing phase.
The writing phase covers the area where at least one thread has not committed himself to the barrier.
Phase C describes the interval in which every thread has written his date, but no thread has left the barrier, yet.
...
The length of the phases A and B is only dependent on the thread count. They are equal for different protocols.
In the Central Counter Barrier the last thread to commit himself very likely immediately exits the barrier. Therefore there is few time, on average, between the last thread writing and the first one leaving the barrier (green).
...
In particular suppose you have $n$ concurrent threads of execution, modelled as interleaved CTMC modules, and in each one transition with rate $\lambda$ is enabled. The average delay of the first transition to fire is then $\frac{1}{n \cdot \lambda}$. The average time of a parallel execution of all $n$ transitions, assuming no interleaving with other transitions, is $\sum_{i=1}^{n} \frac{1}{i \cdot \lambda}$ rather than the intuitive $\frac{1}{\lambda}$.
...
Where the Central Counter Barrier's energy consumption scales similarly to the duration, the B1 Barrier's energy consumption grows faster than the time needed.
...
Both Barriers have very high power consumption between the moment the last thread commits himself and the first one leaves (green). For the Central Counter Barrier this is due to the shortness of this phase. In the B1 Barrier shared read operations are issued almost exclusively. In contrast to shared write operations they are modelled to take half the time, but use the same amount of energy.
\begin{figure}[htbp]
	\centering
...
Figure~\ref{fig:c1-time-work-100-percent} shows how much time writing versus reading takes for both protocols.
Writing begins when the first thread enters the barrier and ends when the last thread announced its arrival.
Reading begins when the last thread committed itself and ends when the barrier completes.
Deviating from our expectation the Central Counter Barrier's ratio is the same across all thread counts. This is due to the CTMC interleaving behaviour, explained earlier in this section.
As expected, from our informal analysis in Section~\ref{sssec:analysis-general-shared}, with increasing thread count the ration of writing to reading time for the B1 Barrier increases.
\begin{figure}[htbp]
	\centering
...
Having reviewed different metrics for a rather short initial work period, we now have a look at similarly organised figures with an increased work period duration of 1000 cycles. Note that in comparison to real life scenarios this is still small. According to \cite{rab00} each process at a barrier spends 8194 process cycles idly\footnote{1813 microseconds accumulated idle time at 450MHz on 99.6 processes}. But an average of 1000 cycles is already enough to emulate threads arriving in intervals large enough so that each can conduct his desired work before the next one arrives.

The basic timing of the barriers is very different from before. In Figure~\ref{fig:c1-time-work-1000-B-C-D} one can observe that the Central Counter Barrier outperforms the B1 Barrier in every respect.
The serialisation of write operations for the B1 Barrier nullifies its advantage in this phase. And, as mentioned before, the reading phase takes longer as well.
The duration of the last thread to enter to the first thread to leave for the Central Counter Barrier converges to 100 cycles, one atomic operation, plus a few additional cycles for other instructions.
\begin{figure}[htbp]
	\centering
...
Phases A and B are excluded because they are large and behave very similarly for both protocols.
\begin{figure}[htbp]
	\centering
	\begin{minipage}{0.53\linewidth}
...
Since changing the work period only influences the phases before each thread has committed his date, the interval between C and D is the same as in Figure~\ref{fig:c1-work-100-partition}.
...
The power consumption behaves similarly to the previous measurement where we used an average work period duration of 100 cycles.
