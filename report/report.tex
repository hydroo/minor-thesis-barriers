...
\begin{itemize}
	...
	\item explain that barriers are very common in concurrent programming. OpenMP's\cite{openmp} implicit barriers. MPI barriers\cite{rab00}. Relate to normal message passing, because numbers without relation might not be very helpfull.
	\item motivation for researching barriers: apply ideas of pW/CS\cite{pwcs} to other synchronization primitives, e.g. barriers, and model check them\cite{bai13}. (reread both. Maybe his intentions are different from ours. Point that out. ?Speed vs Safety, probabilistic correctness vs probabilistic runtime?)
		\begin{itemize}
			\item \todo rework this list. Some of the concept do not matter that much for the barrier case. Not sure if leaving out some things or explicitly stating that it doesn't apply.
			\item \textbf{bai13}
			\item don't avoid inconsistency at all cost. Make errors detectable (or maybe ignorable in the barrier case).
			\item inherent randomness\cite{mcg09}, induced by caching, scheduling, randomness at hardware level. Or short: complexity of today's computer systems.
			\item sufficiently high probability of success
	...
	\item what will we attempt in this report. ?overarching thesis, setup for a continuous thread, or a claim we will pickup again later?
