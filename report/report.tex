...
\begin{itemize}
	\item work = 100. Expected it takes a thread to arrive at the barrier is 100 cycles.
	...
	\item time
		\begin{itemize}
			...
			\item the initial work distribution is the same for all processes and protocols. Therefor
			...
			\item all parts take longer with increasing thread count, since CTMC semantic (described in Section~\ref{sssec:analysis-modelchecking-preliminaries}) makes interleaved transitions get an exit rate of $n \cdot \lambda$ for n modules each with rate $\lambda$, so that the expected time of a parallel execution of all transitions is $\sum_{i=1}^{n} \frac{1}{i \cdot \lambda}$  time rather than $1 / \lambda$ for $n$ interleaved modules / concurrent threads.
			\item for example the expected number of cycles for the first process to enter is $\approx 100 / n$. The last one is finished working after $\approx$ 150, 183 and 208 cycles, for 2, 3 and 4 processes.
...
\begin{itemize}
	...
	\item both new barriers presented show promise.
	...
	\item CTMC model checking semantic for interleaving modules is different from our intuitive notion of parallel execution. Therefore one has to exercise caution when interpreting model checking results. Nevertheless it enabled us to look into barrier algorithms at a more fine-grained level than measurement-based methods could hope to.
