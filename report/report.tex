...
MPI also supports RMA. Although some MPI implementations, for example Open~MPI~\cite{openmpi}, do not support version 3.0 of the specification, we will restrict ourselves to analysing it, because it subsumes the features of version 2.2, it is already a year old and supported by other prominent implementations such as MPICH~3.0~\cite{mpich} and derivatives like MVAPICH2~\cite{mvapich} and Cray MPT~\cite{craympt}.
...
The GNU~OpenMP implementation, included in the GNU Compiler Collection (GCC) version 4.8.0, implements a \emph{Central Counter Barrier}. Figure~\ref{fig:pseudocode-central-counter} describes the algorithm in pseudocode that is executed on each thread taking part in the barrier synchronisation.
The Central Counter Barrier uses the variable \texttt{barrier} to hold the number of threads that do not yet have arrived at the barrier. Initially it is assigned the number of threads. Upon entering the barrier each thread atomically decrements the variable by one and then waits until the variable reaches zero -- all threads have arrived. After the variable reached zero, all threads will eventually realise the fact and leave the barrier.
...
The loop additionally keeps count of the number of unsuccessful reads. If this count reaches a certain amount an operating system mechanism, called futex\cite{franke2002}, is employed to stop the thread until a later time when the other threads hopefully have arrived. Through this the system avoids unnecessary wait-spinning which would negatively impact power consumption. This counter is architecture specific and set to an equivalent of about 3 milliseconds on GNU/Linux. In case the number of threads is larger than the number of physical cores, i.e. the processor is oversubscribed, the counter is set to a much lower value. This way the operating system scheduler can switch between threads more quickly so that threads that need the time are not blocked by wait-spinning ones.

Throughout this thesis we will assume that the thread and process count is at most as high as the number of physical cores in a system, because oversubscription is not meaningful inside a single program and this situation rarely arises.

% glibc/nptl/sysdeps/pthread/pthread.h, glibc/nptl/pthread_barrier_*, glibc/nptl/sysdeps/unix/sysv/linux/internaltypes.h
The GNU POXIS Threads implementation, called Native POSIX Threads Library (NPTL), is part of the The GNU~C~Library.
The basic barrier algorithm of it is the same as for GNU~OpenMP. One main differences is that it does not use an atomic operations to decrement the variable, but explicitly locks it to guard from interfering access. Second there is no spin-waiting, but upon arriving at the barrier the thread will immediately suspend, via a futex, and wait to be woken up by the last thread arriving. The third difference is that to reset the barrier each thread atomically increments the barrier variable by one upon leaving.

%%%%%%%%%%%%%%%%%%%%
\subsubsection{Distributed memory systems}
\label{sssec:background-currently-used-distributed}

In this section we present the barrier algorithms that are used in two major open source MPI implementations, namely Open~MPI~\cite{openmpi} and MPICH~\cite{mpich}. We restrict ourselves to MPI for the same reasons we only considered MPI for analysing in Section~\ref{sssec:background-means-distributed}.
...
Open~MPI distinguishes between process counts that are powers of two and which are not. If it is a power of two a \emph{recursive doubling} barrier is used. A detailed explanation on this protocol can be found in \cite{hoefler2005}. For non-power of two process counts Open~MPI uses the \emph{Dissemination Barrier}~\cite{hensgen1988}.
...
\begin{itemize}
		...
		\begin{itemize}
			\item
				\begin{itemize}
					...
					\item Figure~\ref{fig:pseudocode-dissemination} presents a pseudocode listing of the algorithm.
					\item rounds
					\item tree esque
					\item each round the number of messages is the same
					\item the knowledge of arrived processes doubles for each process each round.
					\item after $\lceil \log_2 n\rceil$ rounds we are done
					\item Figure~\ref{fig:diagram-dissemination} shows the communication pattern through the rounds.
				\end{itemize}

			\item technicalities:
			\item Dissemination uses sync sendrecv
			\item back-off hard to determine. probably through OPAL\_THREAD\_(UN)LOCK in the end
			\item resetting not necessary since sendrecv sets up itself newly everytime.
	...
	\item mpich
		\begin{itemize}
			...
			\item log2 Dissemination with sendrecv. same as Open~MPI
		\end{itemize}
	\item Dissemination is also be used for shared memory\cite{hoefler2013}
\end{itemize}
