...
First we assume the initial work period to take 100 cycles on average, which is extremely low considering that barrier synchronisation takes takes more time than doing actual work. Later we switch to larger work periods.
...
The time from the last thread to enter the barrier to the last thread to leave the barrier (dark red) is similar for both the Central Counter and the B1 Barrier.
...
The diagram presents the four basic points in time we chose, along with the moment the last thread finishes writing, in a stacked fashion.
Phase A covers the accumulated time or energy from the starting point until the first thread entered the barrier.
...
Phase C describes the interval in which every thread has written his date, but no thread has left.
Phase D depicts the accumulated time or energy from the first to the last thread leaving the barrier.
Phase C and D can also be understood as the reading phase, because no more writes are issued.
\begin{figure}[htbp]
	\centering
...
In the Central Counter Barrier the last thread to commit himself very likely immediately exists the barrier. Therefore there is no time between the last thread writing and the first leaving the barrier (green).

As conjectured in Section~\ref{sssec:analysis-general-shared} writing is quicker for the B1 Barrier and reading, i.e. C and D combined, for the Central Counter Barrier.
