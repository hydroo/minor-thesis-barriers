...
\begin{itemize}
	...
	\item distribution of total time and energy spent: Figure~\ref{fig:cs-work-100-partition}
		\begin{figure}[htbp]
			...
			\includegraphics[width=10cm]{charts/cs-energy-work-100-partition}
...
\begin{itemize}
	...
	\item distribution of total time spent and total energy consumed: Figure~\ref{fig:df-work-100-partition}
		...
		\begin{itemize}
			...
			\item last and first exit at almost the same time for dissemination. Explain probably not because boring.
	...
	\item dissemination: distribution of total time spent and energy consumed: Figure~\ref{fig:d-work-100-partition}
		...
		\end{figure}
		\begin{itemize}
			\item stair-esque running time due to $\lceil \log_2 \mathit{processCount} \rceil$ number of rounds.
			\item unfavourable for non-power-of-two processes counts. Ref to Section~\ref{sssec:analysis-general-distributed}
			\item slight increase in per-round execution time with number of processes is result of CTMC interleaving semantics. Can be seen as resource conflicts for remote transfers.
			\item energy consumption increase per round a bit larger than runtime increase, because more remote and local operations are being issued.
		\end{itemize}
	\item rate of power consumption per phase: Figure~\ref{fig:df-power-work-100}
		...
		\end{figure}
		\begin{itemize}
			\item overall relatively similar between the two algorithms
			\item largely because the comparing to the rewards given for operations the amount of energy consumed is similar.
			...
			\item overall ronny's fancy performs a bit better.
		\end{itemize}
	...
	\item distribution of total time spent and energy consumed: Figure~\ref{fig:df-work-1000-partition}
		...
		\begin{itemize}
			...
			\item serialization of arrivals makes the CTMC effect (slight increase in per round time with more processes) for dissemination go away
			\item \todo talk about progress problem
		\end{itemize}
	\item dissemination: distribution of total time spent and energy consumed: Figure~\ref{fig:d-work-1000-partition}
		...
		\begin{itemize}
			\item as expected
			\item serialization makes the additional cost per round almost constant
			\item energy increases with more processes because one more remote operation are issued per round
		\end{itemize}
	\item rate of power consumption per phase: Figure~\ref{fig:df-power-work-1000}
		...
		\end{figure}
		\begin{itemize}
			\item overall power consumption very similar because of the 1000 cycle work rate
