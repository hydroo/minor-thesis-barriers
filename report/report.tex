...
\begin{itemize}
	...
	\item talk about the progress problem when one process is missing.
		\begin{itemize}
			\item normal Figure~\ref{fig:diagram-dissemination} in Section~\ref{sssec:existing-currently-used-distributed}
			\item suppose process 4 has not entered the barrier.
			\item Figure~\ref{fig:diagram-dissemination-progress}
				\begin{figure}[htbp]
					\centering
				...
				\end{figure}
			\item number of blocked processes doubles each round. (same for non-power of two process counts)
			...
			\item who knows of whose arrival Figure~\ref{fig:table-dissemination-progress}
				...
				\end{figure}
			\item ronny's fancy does need multiple rounds to finish. The last arriving thread can fetch all others from one other process. And all others need to one more successful remote read. Those can be done in parallel.
	...
	\item resulting properties
		\begin{itemize}
			...
			\item \textbf{D:} \emph{Average reward until the last thread leaves the barrier}
				...
				\end{center}
			\item \textbf{E:} \emph{Average reward until the last thread finished writing}
				\begin{center}
...
\begin{itemize}
	...
	\item distribution of total time spent: Figure~\ref{fig:cs-work-100-partition}
		\begin{figure}[htbp]
			...
			\includegraphics[width=8cm]{charts/cs-energy-work-100-partition}
		...
		\begin{itemize}
			...
			\item ronny's simple needs more energy. Because it performs more shared memory actions and more local operations in the same amount of time. Doesn't wait for atomic ops as central counter is.
			...
			\item ronny's simple spends considerable energy from the last commitment to first leaving the barrier. Obvious why from the algorithms. (and same as time)
		\end{itemize}
	\item rate of power consumption per protocol phase: Figure~\ref{fig:cs-energy-work-100-phases}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/cs-energy-work-100-phases}
			\caption{Power consumption rate per phase}
			\label{fig:cs-energy-work-100-phases}
		\end{figure}
		\begin{itemize}
			\item (A) all working has 11W - the baseline.
			\item overall low for central counter (16-18.3W), ronny's simple (19.4 - 30.6)
			\item no one left: cc: high because very short, rs: high because many shared reads across many threads.
			\item one left similar because cc: one or zero shared reads, rs: few shared reads, many local reads.
		\end{itemize}
	...
	\item last to last, last to first Figure~\ref{fig:cs-time-work-1000-B-C-D}
		...
		\end{itemize}
	\item distribution of total time spent: Figure~\ref{fig:cs-work-1000-partition}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/cs-time-work-1000-partition}
			\includegraphics[width=8cm]{charts/cs-energy-work-1000-partition}
			\caption{Distribution of total time spent and energy consumed}
		...
		energy
		\begin{itemize}
			\item why does the writing phase for ronny's simple take more energy with more threads? Should be even with central counter shouldn't it?
		\end{itemize}
	\item rate of power consumption per protocol phase: Figure~\ref{fig:cs-energy-work-1000-phases}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/cs-energy-work-1000-phases}
			\caption{Power consumption rate per phase}
			\label{fig:cs-energy-work-1000-phases}
		\end{figure}
		\begin{itemize}
		...
		\end{itemize}
	\item percent Figure~\ref{fig:cs-time-work-1000-percent}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/cs-time-work-1000-percent}
			\caption{Percentage of time spent writing vs time spent reading with 1,000 cycles work period}
			\label{fig:cs-time-work-1000-percent}
		\end{figure}
		\begin{itemize}
...
\begin{itemize}
	...
	\item last to last, last to first Figure~\ref{fig:df-time-work-100-B-C-D}
		...
		\end{itemize}
	\item distribution of total time spent: Figure~\ref{fig:df-time-work-100-partition}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/df-time-work-100-partition}
			\caption{Distribution of total time spent}
			\label{fig:df-time-work-100-partition}
		\end{figure}
		\begin{itemize}
			\item similar picture including work period
		\end{itemize}
	\item dissemination: distribution of total time spent: Figure~\ref{fig:d-time-work-100-partition}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/d-time-work-100-partition}
			\caption{Distribution of total time spent}
			\label{fig:d-time-work-100-partition}
		\end{figure}
		\begin{itemize}
		...
		\end{itemize}
	\item progress problem of the dissemination barrier: Figure~\ref{fig:d-time-work-1000-partition}
		\begin{figure}[htbp]
			\centering
			\includegraphics[width=8cm]{charts/d-time-work-1000-partition}
			\caption{Distribution of total time spent}
			\label{fig:d-time-work-1000-partition}
		\end{figure}
		\begin{itemize}
