...
\begin{itemize}
	\item which parameters to variate
		\begin{itemize}
			\item thread count. Maximum 4 is considered (maybe more for diss vs fancy). Model checking limitations bla.
	...
	\item resulting properties
		...
		\end{itemize}
	\item properties of dissemination and ronny's fancy
		\begin{itemize}
			\item progress
		...
		\item central counter vs ronny's simple
			\begin{itemize}
				...
				\item distribution of total time spent: Figure~\ref{fig:cs-work-100-partition}
					...
					\begin{itemize}
						...
						\item first thread leaves a few cycles after writing is done. Because the last one to write its value will
							read the completed barrier and immediately leave.
				...
				\item reading vs writing: Figure~\ref{fig:cs-work-100-percent}
					...
					\item writing is expected to take less time than shown here, since CTMC semantic makes interleaved transitions get an exit rate of $\lambda / n$ for n modules each with rate $\lambda$, so that the expected time of a parallel execution is $\sum_{i=1}^{n} \frac{1}{i \cdot \lambda}}$  time rather than just $1 / \lambda$ for $n$ interleaved modules / concurrent threads
					\end{itemize}
				...
				\item last to last, last to first Figure~\ref{fig:cs-work-1000-B-C-D}
					...
					\begin{itemize}
						...
						\item central counter fast in all respects.
				...
				\item percent Figure~\ref{fig:cs-work-1000-percent}
					...
					\begin{itemize}
						...
						\item ronny's simple waiting time increases linearly with the number of threads (2:77, 3:145, 4:208 cycles)
					\end{itemize}
				\item with larger and more realistic, since 1000 cycles is still very few, work periods this problem would increase for low thread counts.
			\end{itemize}
