...
Many synchronisation operations imply a barrier. For example the parallel for loop of the OpenMP~\cite{openmp} programming interface makes sure that no thread continues program execution until all iterations are completed, since the following operations might rely on the results of the loop. Likewise in MPI~\cite{mpiforum}, global operations like reduce, scatter and gather imply a barrier synchronisation.
...
Therefore, before entering the next simulation phase, information, like temperature, humidity or cloud movement, about these regions is exchanged.
...
\begin{itemize}
	...
	\item Because of the inherent randomness~\cite{mcg09}, induced by caching, scheduling, memory access latency differences, or short: the complexity of today's computer systems, one can view concurrent memory access as genuinely random. This enables the use of probabilistic algorithms for development, and the tools of probability theory for analysis.
\end{itemize}
One wants to design algorithms that have a sufficiently high probability of success, tolerate faulty behaviour and, on average, perform better than their deterministic counterparts.
...
Testing, as well as measurement, is usually deterministic. When testing probabilistic algorithms deterministically, there is always a chance of errors not being revealed. Randomising tests only gives a certain degree of confidence as well. Both methods do not guarantee full coverage.
...
The structure of this work is as follows. First, we survey the tools developers have at their disposal to implement barriers. Second, we present today's mainly used barrier algorithms. To conclude the background study, we give an account of the basic anatomy of barriers.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Means to Implement Barriers}
...
A parallel program for shared memory systems consists of multiple threads of execution -- short \emph{threads}.
...
Two ways one could implement such algorithms are via the User Datagram Protocol (UDP)~\cite{udp} and InfiniBand's unreliable connections.
...
MPI also supports RMA. Although some MPI implementations, for example Open~MPI~\cite{openmpi}, do not yet support version 3.0 of the interface specification, we will restrict ourselves to analysing it, because it subsumes the features of version 2.2~\cite{mpi2}, it is already a year old and supported by other prominent implementations such as MPICH~3.0~\cite{mpich} and its derivatives MVAPICH2~\cite{mvapich} and Cray MPT~\cite{craympt}.
...
Normally a processor commits 32 or 64 bits to main memory atomically, and in the remainder of this thesis we will assume this is the case.
...
Open~MPI's barrier implementation distinguishes between process counts that are powers of two and which are not. If it is a power of two, a \emph{recursive doubling} barrier is used. A detailed explanation of this protocol can be found in \cite{hoefler2005}. For process counts that are not a power of two Open~MPI uses the \emph{Dissemination Barrier}, introduced by Hensgen, Finkel and Manber in 1988~\cite{hensgen1988}.

Figure~\ref{fig:pseudocode-dissemination} presents the pseudocode of this algorithm, which is executed on each process individually. A process is identified by its \emph{process index}, which is a number between 0 and $\mathit{processCount} - 1$.
For readability reasons we have chosen a representation of the protocol where read/write operations use local or remote memory depending on where the accessed array element is located. The protocol can similarly be implemented using message passing techniques.
...
One independent piece is the employed way of reinitialising the barrier after use. As we have seen for the Central Counter Barrier (Section~\ref{sssec:background-current-shared}) resetting might be achievable through slight modification of the algorithm. This does not work for all protocols, though.
...
\label{ssec:innovative-mgb}
The original idea of the following algorithm is part of a private communique between Nicholas Mc~Guire, the pW/CS lock inventor, and us. Figure~\ref{fig:pseudocode-mgb} presents the new barrier, which is a variation of the protocol first mentioned in \cite{bre13}. It is intended for shared memory architectures.
...
Figure~\ref{fig:pseudocode-b2} presents the B2 Barrier's pseudocode. The code uses the same remote memory access style as the Dissemination Barrier (Figure~\ref{fig:pseudocode-dissemination} in Section~\ref{sssec:background-current-distributed}), i.e. read/write operations use local or remote memory depending on where the accessed array element is located. The notation used in this listing is explained in Section~\ref{ssec:innovative-mgb}.
\begin{figure}[htbp]
	\centering
	\input{listings/b2}
...
Like the MGB Barrier this protocol is restricted to 64 processes. An extension to arrays of bit sets can be implemented in the same way as described in Section~\ref{ssec:innovative-mgb}.
...
The Central Counter Barrier (Figure~\ref{fig:pseudocode-central-counter} in Section~\ref{sssec:background-current-shared}) and the B1 Barrier (Figure~\ref{fig:pseudocode-b1} in Section~\ref{ssec:innovative-b1}) are both divided into two phases: the reading and the writing phase.
...
The memory required by the Central Counter Barrier is logarithmic in the number of threads. With a 64-bit counter up to $2^{64}$ threads can be synchronised. The B1 Barrier uses a linear amount of memory -- $\mathit{threadCount} \cdot 64$ bytes, assuming a cache line is 64 bytes large and disregarding the local variable. This might seem like a lot, but since shared memory systems rarely exceed 512 cores, a maximum of only 32 kibibytes is used.
...
The Dissemination Barrier (Figure~\ref{fig:pseudocode-dissemination} and \ref{fig:diagram-dissemination} in Section~\ref{sssec:background-current-distributed}) and the B2 Barrier (Figure~\ref{fig:pseudocode-b2} in Section~\ref{ssec:innovative-b2}) communicate very differently.
...
Requiring $\lceil \log _2~n \rceil$ rounds is obviously worse than $\log _2~n$. Especially from $n$, where $n$ is a power of two, to $n+1$ processes the number of rounds increases by one. Therefore, the runtime of the Dissemination Barrier is expected to show a stair-esque behaviour. Every time the number of rounds increases the duration of the barrier increases by a large step, whereas adding more processes to the same number of rounds adds little overhead.
In contrast to this the execution time of the B2 Barrier is distributed evenly.

Synchronisation using the Dissemination and B2 Barrier requires the same amount of memory. In principle $2 \cdot \lceil \log _2~n \rceil$ bits per process suffice for both algorithms. The Dissemination variant shown in Figure~\ref{fig:pseudocode-dissemination} uses a total of $n \cdot (n + \lceil \log _2~n \rceil)$ bits, but, since each process receives from only $\lceil \log _2~n \rceil$ other processes, a modified array index calculation would allow the protocol to work with $n \cdot 2 \cdot \lceil \log _2 ~n \rceil$ bits.
...
The B2 Barrier does not work in rounds and does not have designated communication partners. It therefore fetches all other process' arrival notifications while one process is missing.
...
Instead of naming states, we oftentimes use state predicates, combined using propositional formulas, to describe sets of states. Their meaning will be obvious from the context.
...
\begin{figure}[htbp]
	...
	\caption{CTMC for one cache line copy}
...
The B1 Barrier (Figure~\ref{fig:pseudocode-b1} in Section~\ref{ssec:innovative-b1}) allocates one cache line per array element. Therefore, the final model is composed of $n$ algorithm modules plus $n^2$ cache line copy modules where $n$ is the number of threads.
...
\end{figure}
Local operations take far less time than remote operations -- between ten to a thousand times usually. Therefore, we assign the rate $\lambda_c$ (one processor clock cycle) to local operations.
...
The B2 Barrier, introduced in Section~\ref{ssec:innovative-b2}, is modelled as a composition of one algorithm module per process.
...
The following, very limited assumptions are based on a sample measurement conducted on a personal laptop.
We use a processor clock rate of 2.5 GHz to convert between seconds and cycles.
...
All phases take longer with increasing thread count, because of the CTMC semantic for simultaneously enabled transitions, described in Section~\ref{sssec:analysis-modelchecking-preliminaries}. The exit rate of a transition is the sum of the rates of all currently enabled transitions.
...
The Dissemination Barrier finishes 23 cycles earlier for two processes, because remote writes can be issued during another process' work period, whereas the B2 Barrier needs to execute two more remote read operations after the last process enters the barrier.
...
In this work we present three innovative barrier protocols. We compare two of them to two state-of-the-art protocols using model checking techniques. The B2 Barrier, having a shorter execution time and lower energy consumption than the Dissemination Barrier, turns out to be a promising algorithm.
...
Model checking enables exhaustive and fine-grained analysis beyond what measurement-based methods deliver.
...
The possible future directions of this work can be divided into five categories.
