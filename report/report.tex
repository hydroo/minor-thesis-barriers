...
\begin{itemize}
	...
	\item explain the basic pseudo code listing style somewhere
		\begin{itemize}
	...
	\item we assume that one barrier per thread team is enough. There is no use to having two barrier data structures for the same team. When we talk about memory consumption this is important. -- is already mentioned in~\ref{sssec:analysis-general-shared}
\end{itemize}
...
\begin{itemize}
	...
	\item gnu openmp\cite{gomp}
		\begin{itemize}
			...
			\item back-off via a spin-counter $\sim$ 3ms. architecture specific. lowers spincount if core count $<$ thread count. futexes\cite{franke2002}. kernel based mutexes. wait queues. sleeping.
...
\begin{itemize}
	...
	\item reset
		...
		\end{itemize}
	\item back-off
	\item we can combine barrier protocols. Explain how. Reap benefits, mitigate shortcomings. Adapt to topology (explain. Node shared mem, internode dist mem., different interconnect topologies) aware.
\end{itemize}
