...
\begin{itemize}
	...
	\item first appearance in bre13\cite{bre13}. Slight modification since then "remembers others" considered here.
		\begin{figure}[htbp]
			\centering
...
\begin{itemize}
	...
	\item one date per cache line to avoid false sharing (?citation?, ?explanation?)
	\item advantage: in contrast to the central-counter barrier the assignments to shared variables can be done in parallel. Elaborate.
	\item many read accesses, but since data is cached the latency is only significant once per cache line / date. i.e. when the thread arrives and writes his date to be \texttt{true}.
	\item advantage: no atomic ops
	\item much shared read communication but low computation overhead
	\item The barrier is destructive in the sense that it cannot be executed two times in a row. One way to circumvent this is to modify the algorithm to use a repetition counter instead of a boolean (since we have one date per cache line we allocated the space anyways) and keep track of it. The algorithm would then atomically increase the counter instead of the assignment in line 4.
...
\begin{itemize}
	...
	\item Figure~\ref{fig:ronny-fancy-no-reset}
		...
		\end{figure}
	\item each array element is located in the memory of the respective process. Distributed memory.
	\item both protocols work for shared memory and remote memory access.
	...
	\item discuss resetting through triple barrier. Perhaps, briefly hint that this kind of resetting can be added to any barrier, more on this in the section on the orthogonal design of barriers.
